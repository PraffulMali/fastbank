...
class AccountService:
    ...
    async def get_my_accounts(db: AsyncSession, current_user: User):
        ...
        return {
            "accounts": [
                AccountUserSingleResponse.model_validate(acc) for acc in accounts
            ]
        }

    @staticmethod
    async def get_account_with_permissions(
        db: AsyncSession, account_id: uuid.UUID, tenant_id: uuid.UUID
    ) -> Account:
    ...
    async def update_account(
        db: AsyncSession, account_id: uuid.UUID, account_update: AccountUpdate
    ) -> Optional[Account]:
    ...
    async def soft_delete_account(
        ...
        if account.balance > 0:
            raise ValueError(
                "Cannot delete account with non-zero balance. Please withdraw funds first."
            )
