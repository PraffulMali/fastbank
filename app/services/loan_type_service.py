...
class LoanTypeService:
    ...
    async def create_loan_type(
        db: AsyncSession, loan_type_in: LoanTypeCreate, tenant_id: uuid.UUID
    ) -> LoanType:
        ...
        if result.scalar_one_or_none():
            raise ValueError(
                f"Loan type '{loan_type_in.name}' already exists in this tenant"
            )

        new_loan_type = LoanType(
            tenant_id=tenant_id, name=loan_type_in.name, is_active=True
        )
    ...
    async def delete_loan_type(
        ...
        loans_query = (
            select(func.count())
            .select_from(Loan)
            .where(Loan.loan_type_id == loan_type_id)
        )
        ...
        rules_query = (
            ...
            .where(InterestRule.loan_type_id == loan_type_id)
        )
    ...
    async def get_loan_type_with_rate(
        ...
        return {
            ...
            "interest_rate": (
                float(interest_rule.interest_rate) if interest_rule else None
            ),
        }
